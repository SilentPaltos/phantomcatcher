    ...
    execute \
        on passengers \
        run data \
            modify entity @s item.components."minecraft:item_model" \
            set value "phantomcatcher:entity/phantomcatcher"

        # Заряд 0
        execute \
            on passengers \
            run data \
                modify entity @s item.components."minecraft:custom_model_data" \
                set value { floats: [0.0f] }
