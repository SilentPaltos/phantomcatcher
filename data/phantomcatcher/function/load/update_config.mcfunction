    ...
    # Версия данных датапака
        ...
        scoreboard players set *version phantomcatcher.config 1
    ...
    # Период зарядки
        ...
        run \
            data get storage phantomcatcher charge_period
    ...
    # Может ли ловец поймать переименованных сущностей
        ...
        run \
            data get storage phantomcatcher catch_renamed_catchables
    ...
    # Охватываемый ловцом квадрат
        ...
        run \
            data get storage phantomcatcher catch_square_size
        ...
        run \
            scoreboard players set *catch_square_dstart phantomcatcher.config 0
        
        execute \
            if score *catch_square_dstart phantomcatcher.config matches 1.. \
        run \
            scoreboard players operation *catch_square_dstart phantomcatcher.config *= *16 phantomcatcher.config
        ...
        run \
            data get storage phantomcatcher catch_square_size 2
        ...
        run \
            scoreboard players operation *catch_square_diagonal phantomcatcher.config *= *16 phantomcatcher.config
        ...
        execute \
            if score *catch_square_diagonal phantomcatcher.config matches ..-1 \
        run \
            scoreboard players set *catch_square_diagonal phantomcatcher.config 0
